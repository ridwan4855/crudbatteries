...
<div class="bg-white rounded-lg shadow p-6 max-w-2xl mx-auto">
    <h2 class="text-2xl font-bold mb-6">
        <?= isset($battery) ? 'Edit Battery' : 'Add New Battery' ?>
    </h2>
    ...
    <form action="<?= isset($battery) ? "/batteries/update/{$battery['id']}" : '/batteries/store' ?>" method="POST">
        <?= csrf_field() ?>

        <?php if (isset($battery)) : ?>
        ...
        <div class="mb-4">
            ...
            <select name="type" class="w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <option value="<?= $type['id'] ?>"
                    <?= (string)$type['id'] === (string)old('type', $battery['type'] ?? '') ? 'selected' : '' ?>>
                    <?= $type['type_name'] ?>
                </option>
                <?php endforeach ?>
            </select>
        ...
        <div class="flex justify-end gap-2">
            ...
            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                <?= isset($battery) ? 'Update Battery' : 'Save Battery' ?>
            </button>
        </div>
    </form>
</div>
